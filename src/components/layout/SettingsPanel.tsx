    ...
    <>
      ...
      <motion.div
        ...
        <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400">
              <span>Current plan</span>
              <span className="font-medium text-slate-700 dark:text-slate-300 capitalize">{user?.subscriptionTier || 'Free'}</span>
            </div>
            <button
              onClick={async () => {
                try {
                  const data = await api.stripe.createPortalSession();
                  window.open(data.url, '_blank');
                  onClose();
                } catch (err: any) {
                  toast.error(err.message || 'Unable to open subscription management');
                }
              }}
              className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-left">
              <CreditCardIcon className="h-4 w-4 text-slate-400 dark:text-slate-500" />
              Manage Subscription
            </button>
          </div>
        </div>
