...
router.post('/admin/restart-processing', authenticateAdmin, async (req, res: Response) => {
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      const r = results[i];
      console.log(`[Manual Restart] ${r.fromCheckpoint ? 'Resuming from checkpoint' : 'Restarting from beginning'}: "${r.filename}" (ID: ${r.id})`);

      if (i > 0) {
        await new Promise(resolve => setTimeout(resolve, 5000));
      }

      processTranscription(r.id).catch(err => {
        console.error(`[Manual Restart] Failed for transcript ${r.id}:`, err);
      });
    }
  } catch (err) {
