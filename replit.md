...
- App is installable from Safari (Add to Home Screen) on iOS and from Chrome on Android
- Uses `vite-plugin-pwa` with Workbox `generateSW` mode for service worker generation
- Manifest at `public/manifest.webmanifest` (auto-generated by vite-plugin-pwa from `vite.config.ts`)
- Icons: `public/pwa-192x192.png`, `public/pwa-512x512.png`, `public/apple-touch-icon.png`, `public/icon.svg`
- Offline: SPA shell is precached; when offline, the app shell loads from cache and the app's own error handling covers API failures
- `public/offline.html` — standalone offline page (available as a static fallback)
...
- Service worker auto-updates on new deployments (`registerType: 'autoUpdate'`)
